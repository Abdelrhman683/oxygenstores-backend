                            ...
                            <div
                                ...
                                <div class=" bg-white select-method-border rounded">
                                    @if($isPhysicalProductExist && $shippingMethod=='sellerwise_shipping' && $shipping_type == 'order_wise')
                                        ...
                                        @if((isset($chosenShipping)==false || $chosenShipping['shipping_method_id']==0) && count($shippings) > 0)
                                            <?php
                                                $firstShipping = $shippings->first();
                                                $chosenShipping = \App\Models\CartShipping::updateOrCreate(
                                                    ['cart_group_id' => $cartItem['cart_group_id']],
                                                    [
                                                        'shipping_method_id' => $firstShipping['id'],
                                                        'shipping_cost' => $firstShipping['cost'],
                                                        'created_at' => now(),
                                                        'updated_at' => now(),
                                                    ]
                                                );
